...
public class MyMediaRecorder {
    ...
    public MyMediaRecorder(int width, int height, EGLContext eglContext,
                           Context context) {
        mWidth = width;
        mHeight = height;
    ...
    private Uri createVideoUri() {
        ...
        values.put(MediaStore.Video.Media.DISPLAY_NAME, "MyVideo_" + System.currentTimeMillis() + ".mp4");
    ...
    public void start(float speed) throws IOException {
        ...
        try {
            pfd = getFileDescriptor(videoUri);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }

        // 4, 创建封装器
        mMediaMuxer = new MediaMuxer(pfd.getFileDescriptor(), MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);
